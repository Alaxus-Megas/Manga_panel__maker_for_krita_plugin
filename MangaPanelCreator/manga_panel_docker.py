...
class CustomGridDialog(QDialog):
    def __init__(self, parent=None, total_rows=2, current_config=None):
        super().__init__(parent)
        self.setWindowTitle("Custom Layout Configuration")
        self.resize(300, 400)
        self.row_spinboxes = []
        self.result_config = []
        
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel(f"Configure Columns for {total_rows} Rows:"))
        ...
        for i in range(total_rows):
            ...
            row_layout.addWidget(QLabel(f"Row {i+1} Columns:"))
            spin = QSpinBox()
            spin.setRange(1, 10)
          
            val = current_config[i] if current_config and i < len(current_config) else 1
            spin.setValue(val)
        ...
        btn_box = QHBoxLayout()
        btn_ok = QPushButton("Ready / Listo")
    ...
    def save_and_close(self):
        self.result_config = [spin.value() for spin in self.row_spinboxes]
        self.accept()
...
class MangaPanelDocker(DockWidget):
    ...
    def open_custom_grid_dialog(self):
        ...
        dialog = CustomGridDialog(self, total_rows=rows, current_config=self.custom_row_config)
        if dialog.exec_():
            self.custom_row_config = dialog.result_config
            self.use_custom_grid = True
            self.btn_custom_rows.setStyleSheet("background-color: #00FF00; color: black; font-size: 10px;")
            QMessageBox.information(self, "Custom Mode", "Custom layout saved!\nClick 'GENERATE GRID PAGE' to apply.")
        else:
            pass
    ...
    def create_grid_panels(self):
        ...
        is_custom = self.use_custom_grid and len(self.custom_row_config) == rows
        ...
        for r in range(rows):
            ...
            if is_custom:
                cols_this_row = self.custom_row_config[r]
            else:
                cols_this_row = self.spin_cols.value()
            
          
            total_gutter_w = (cols_this_row - 1) * gutter
            
           
            if available_w <= total_gutter_w:
                current_y += panel_h + gutter
                continue 
                
            panel_w = (available_w - total_gutter_w) / cols_this_row
